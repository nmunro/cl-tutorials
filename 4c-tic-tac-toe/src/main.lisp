...
(defun game-over-p (board)
  (flet ((draw-p (board)
    ...
    (cond
      ; Rows
      ((and (meq:with-multiple-equal (aref board 0 0) (aref board 0 1) (aref board 0 2))
            (not (eql (aref board 0 0) '-)))
       t)

      ((and (meq:with-multiple-equal (aref board 1 0) (aref board 1 1) (aref board 1 2))
            (not (eql (aref board 1 0) '-)))
       t)

      ((and (meq:with-multiple-equal (aref board 2 0) (aref board 2 1) (aref board 2 2))
            (not (eql (aref board 2 0) '-)))
       t)

      ; Columns
      ((and (meq:with-multiple-equal (aref board 0 0) (aref board 1 0) (aref board 2 0))
            (not (eql (aref board 0 0) '-)))
       t)

      ((and (meq:with-multiple-equal (aref board 0 1) (aref board 1 1) (aref board 2 1))
            (not (eql (aref board 0 1) '-)))
       t)

      ((and (meq:with-multiple-equal (aref board 0 2) (aref board 1 2) (aref board 2 2))
            (not (eql (aref board 0 2) '-)))
       t)

      ; Diagonals
      ((and (meq:with-multiple-equal (aref board 0 0) (aref board 1 1) (aref board 2 2))
            (not (eql (aref board 0 0) '-)))
       t)

      ((and (meq:with-multiple-equal (aref board 0 2) (aref board 1 1) (aref board 2 0))
            (not (eql (aref board 0 2) '-)))
       t)
